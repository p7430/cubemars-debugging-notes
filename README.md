...
The "Upper Computer" is CubeMars' proprietary software used to setup, calibrate, update the firmware, and test the motors. It defaults to Chinese, and the button in the bottom left switches it to English.
...
2. Connect them to your controller. I'm using a Jetson Orin Nano, so I attached my CAN Bus Transceiver to the Jetson Orin Nano, connected the CAN High and CAN Low to the actuator(s). You will notice that each actuator has two CAN H and two CAN Low ports (so four total, all next to eachother). One pair is input; the other pair is output. On the output CAN H and CAN L of the last actuator (I had two acctuators in my setup), I added the 120 Ω terminating resistor connecting CAN High and CAN Low. If you're unfamiliar with how CAN bus actually works, I highly recommend watching some videos to understand the setup and why you need a terminating resistor. Double check with a multimeter that once you add the terminating resistor across CAN High and CAN Low it reads 60 Ω.
- This is a video overview of CAN Bus and how it works: https://www.youtube.com/watch?v=JZSCzRT9TTo
...
- A higher level controller. It's a point to point quintic (5th-order polynomial) trajectory planner. This is important because if you tell the motor to move to, say 10°, it will move there at full speed, which is... dangerous. On the other hand, the planner smoothes the motion and makes it safe.
...
1. The AK series motors are pretty powerful, so when you start testing them, I HIGHLY recommend not having anything attached to them as they may move unpredictably. When I first started experimenting with these I naively did have some stuff attached to one (a metal rod attached, normal to the axis of rotation). Now I have a dent in my wall to show for it.
2. This is meant for educational purposes only, it is not meant to be advice (AKA don't sue me)
